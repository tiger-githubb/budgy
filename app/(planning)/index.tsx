...
export default function HomeScreen() {
    ...
    const { lists, settings, archiveLists, deleteLists, resetStore } = useStore();
    ...
    const activeLists = useMemo(() => {
        return lists
            .filter(l => !l.isArchived && typeof l.name === 'string')
            .sort((a, b) => b.createdAt - a.createdAt);
    }, [lists]);

    const hasCorruptedData = useMemo(() => {
        return lists.some(l => typeof l.name !== 'string');
    }, [lists]);

    const { totalBudget, totalSpent } = calculateGlobalTotals(activeLists);

    const handleCreate = () => {
        router.push(routes.planning.list.form);
    };
    ...
    const handlePress = (listId: string) => {
        if (isSelectionMode) {
            ...
            router.push(routes.planning.list.details(listId));
        }
    };
    ...
    useEffect(() => {
        if (hasCorruptedData) {
            Alert.alert(
                "Données corrompues",
                "Des données invalides ont été détectées. Voulez-vous réinitialiser l'application ?",
                [
                ...
                <Card onPress={() => handlePress(item.id)} selected={isSelected}>
                    ...
                    >
                        ...
                        <View style={[styles.cardHeader, isSelectionMode && { marginLeft: 30 }]}>
                            <Text style={[styles.listName, { color: colors.text.primary }]}>{displayName}</Text>
                            {isOverBudget && (
    ...
    const ListFooter = () => (
        <View style={styles.footerContainer}>
            <TouchableOpacity
                ...
                onPress={() => router.push(routes.planning.archived)}
            >
