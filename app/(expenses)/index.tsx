...
export default function ExpensesHome() {
  ...
  // Filter queue to show only if date matches selected month?
  // Usually user wants to see them immediately regardless of date, or usually they are 'recent'.
  // Let's just show them all in a specific "En attente de synchro" section at the top.
  ...
  const sections = useMemo(() => {
    ...
    return Array.from(grouped.entries())
      ...
      .map(([date, items]) => ({
        ...
        total: items.reduce(
          (sum, e) => sum + Number((e as PersonalExpense).amount),
          0,
        ),
      }));
  }, [expenses]);
    ...
    <Animated.View entering={FadeInDown.delay(index * 30).springify()}>
      ...
      >
        ...
        <View style={styles.expenseInfo}>
          <Text style={[styles.expenseTitle, { color: colors.text.primary }]}>
            {item.title}
          </Text>
          <Text
            style={[styles.expenseCategory, { color: colors.text.tertiary }]}
          >
  ...
  const renderOfflineItem = ({ item, index }: { item: any; index: number }) => (
    <Animated.View entering={FadeInDown.delay(index * 30).springify()}>
      <View
        style={[
          styles.expenseItem,
      ...
      >
        ...
        >
          {/* Try to find emoji from category id if possible, else default */}
    ...
    <ScreenWrapper>
      ...
      <Animated.View entering={FadeInUp.delay(150).springify()}>
        <Card style={styles.totalCard}>
          <View style={styles.totalCardContent}>
            <View>
              <Text
                style={[styles.totalLabel, { color: colors.text.tertiary }]}
              >
                Total du mois
              </Text>
              <Text
      ...
      {queue.length > 0 && (
        <View style={styles.offlineSection}>
          ...
          >
            En attente de synchronisation ({queue.length})
          </Text>
          {queue.map((item, index) => (
            <View key={item.tempId} style={{ marginBottom: 8 }}>
              {renderOfflineItem({ item, index })}
            </View>
          ))}
        </View>
      )}
